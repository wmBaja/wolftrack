...
### 2. Setup debugging hotspot connection info
It is useful to be able to connect to the Raspberry Pi over SSH for debugging purposes.  Oftentimes you'll want to log into the Pi through your phone using an app like Termius, and it's simplest if the Pi can just connect to a mobile hotspot from the phone.  If you add Wifi connection information to the Pi with a known SSID and password and the highest priority, then it will automatically connect to that network whenever it's present.  You can do this by editing /etc/wpa_supplicant/wpa_supplicant.conf by running `sudo nano /etc/wpa_supplicant/wpa_supplicant.conf` and adding the following info:
```
network={
  ssid="mynetwork"
  psk="password"
...
### 4. Enable SSH, Serial Port, and Serial Console
Run the Raspberry Pi Configuration tool and enable SSH, Serial Port, and Serial Console.
### 5. Make sure that the Pi's time is automatically synced through NTP
...
### 7. Download the Wolftrack software
...
### 8. Setup a Python virtual environment for Wolftrack
...
### 9. Install the Wolftrack systemd service
